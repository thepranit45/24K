...
from .models import Booking, BusinessHour, Payment, Service
...
def _booked_times(booking_date: date):
...
def about(request):
    ...
    return render(request, 'about.html', {'business_hours': business_hours})
...
def get_time_slots(request):
    ...
    booked = _booked_times(slot_date)
...
def create_booking(request):
    ...
    if already_booked:
        return JsonResponse({'error': 'This slot is already booked. Please choose another.'}, status=409)
    ...
    booking = Booking.objects.create(
        ...
        amount=service.price,   # Always from DB
    ...
    # Track in session so guests can view their booking
...
# ──────────────────────────────────────────────────────────────
# Payment  — NO login required
# ──────────────────────────────────────────────────────────────

def _can_access_booking(request, booking):
    """Check if the current request has access to a booking (owner or session guest)."""
...
# ──────────────────────────────────────────────────────────────
# Confirmation  — NO login required
# ──────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────
# Barber Agenda (Dashboard)
# ──────────────────────────────────────────────────────────────

def barber_agenda(request):
    """Shows all ongoing and upcoming bookings for the day."""
    today = timezone.localdate()
    now_time = timezone.localtime(timezone.now()).time()
    
    # Let's show today's bookings, ordered by time.
    bookings = Booking.objects.filter(
        booking_date=today,
        status__in=[Booking.STATUS_PENDING, Booking.STATUS_CONFIRMED]
    ).select_related('service').order_by('booking_time')
...
# ──────────────────────────────────────────────────────────────
# My Bookings  — shows session bookings for guests, user bookings for logged-in
# ──────────────────────────────────────────────────────────────
...
def booking_list(request):
    ...
    if request.user.is_authenticated:
        # Logged-in: show all their bookings
        qs = Booking.objects.filter(user=request.user).select_related('service', 'payment')
    else:
        # Guest: show only session-tracked bookings
        session_ids = _session_booking_ids(request)
        qs = Booking.objects.filter(booking_id__in=session_ids).select_related('service', 'payment')
    ...
    if status_filter == 'ALL':
        ...
        next_booking = next_qs.select_related('service').order_by('booking_date', 'booking_time').first()
...
def booking_detail(request, booking_id):
    booking = get_object_or_404(
        Booking.objects.select_related('service', 'payment'),
